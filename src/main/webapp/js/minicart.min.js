!function a(b, c, d) {
    ...
    15: [function (a, b) {
        ...
        b.exports = function (a, b) {
            ...
            for (let i = arr.length - 1; i >= 0; i--) {
                if (count != arr.length && (count * 1) % 3 == 0) {
                    price = `,${arr[i]}${price}`;
                } else {
                    price = `${arr[i]}${price}`;
            ...
            return b && b.format && (j = f + price + g), i && (j += " " + d), j
        }
    }, {}],
    16: [function (a, b) {
        "use strict";
    ...
    20: [function (a, b) {
        ...
        var c = b.exports = function (a, b) {
            ...
            d = c.prototype;
        d.load = function () {
            if ("object" == typeof window && window.sessionStorage) {
                var a, b, c = window.sessionStorage.getItem(this._name);
            ...
            if ("object" == typeof window && window.sessionStorage) {
                ...
                }, window.sessionStorage.setItem(this._name, encodeURIComponent(JSON.stringify(b)))
            }
        }, d.destroy = function () {
            "object" == typeof window && window.sessionStorage && window.sessionStorage.removeItem(this._name)
        }
    }, {}],
    21: [function (a, b) {
        "use strict";
...
console.log(getCartFromsessionStorage());


function getCartFromsessionStorage() {
    return JSON.parse(decodeURIComponent(sessionStorage.getItem('PPMiniCart')));
}

function getImage() {
    var divImage = document.querySelector('.minicart-details-image_1');

    // var getIdItem = $('.minicart-details-image_1').data("id");
    // divImage.innerText = getCartFromsessionStorage().value.items[getIdItem].image;
    return divImage;
}
...
function decreasesQuantity2() {
    var arrquantity = document.querySelectorAll('.minicart-quantity');
    var cartTemp = getCartFromsessionStorage();
    for (const value of arrquantity.values()) {

        if (value.value > 1) {
            var quantity = value.value--
            for (let i = 0; i < cart.length; i++) {
                if (value.classList[2] == cart[i].id_item) {
                    cartTemp.value.items[i].quantity -= 1
                }
            }
            sessionStorage.removeItem('PPMiniCart')
            sessionStorage.setItem('PPMiniCart', encodeURIComponent(JSON.stringify(cartTemp)))
        }
    }
}
...
function increaseQuantity2() {
    ...
    document.getElementsByClassName(".change-cart-quantity").value = quantity;
...
function sumMoney() {
    ...
    for (let i = 0; i < cart.length; i++) {
        res += cart[i].quantity * cart[i].amount
    }
...
function addBtnCart() {
    var pathHostName = window.location.pathname
    pathHostName = pathHostName.split('/')
    var pathName = "";
    for (let i = 1; i < pathHostName.length - 1; i++) {
        pathName += `${pathHostName[i]}/`;
    }
    pathName = 'cart'
    if ($("#PPMiniCart form .minicart-footer").length) {
        setTimeout(() => {
            if ($("#PPMiniCart form .minicart-footer .btn-add-cart").length == 0) {
                var createBtnCart = `<a class="btn btn-primary btn-add-cart" href="${pathName}">Xem giỏ hàng</a>`
                $("#PPMiniCart form .minicart-footer").append(createBtnCart);
            }
        }, 10);
    }
}
